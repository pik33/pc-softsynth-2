...
implementation
 const zerosample:TFloatsample=(0,0);
...
function cheby2(input:TFloatsample;mode:integer):Tfloatsample; forward;
...
function resample44to192(from,too:Pfloatsample;il:integer):integer; forward;
...
repeat
  ...
  main2 ;
    ...
    mx:=dpeek($6002c)-64*peek($70002); my:=dpeek($6002e)-40*peek($70002);

    if (peek($60030)=1) and (mx>fs.x+2) and (mx<fs.x+442) and (my>fs.y+30) and (my<fs.y+462) then
      begin
//      poke($60030,0);
      i:=(my-30-fs.y) div 16;
      ...
      if (juzmoznagrac=-1) and (peek($60033)=2) then begin poke($60030,0); poke($60028,13); end;
      end;
...
if juzmoznagrac=-1 then plb.unselect;
if juzmoznagrac=-1 then plb.checkall;

if ((plb.clicked=1)  or (peek($60028)=13)) and (juzmoznagrac=-1) then if filenames[sel+selstart,1]<>'    [DIR]' then
  begin
  plb.clicked:=0;
//  poke($82000,1);
//  repeat until peek($82000)=0;
  ...
  plb.select;
  plb.highlight;
  fh3:=fileopen(currentdir2+filenames[sel+selstart,0],$40);
  wsfn:=currentdir2+filenames[sel+selstart,0];
  //determine file params
  ...
  if head.data<>1635017060 then begin
   i:=0;
    ...
    if k=1635017060 then
      ...
      begin
      messagedlg('Error - wave file invalid', MtError, [MbOk],0);
      fileclose(fh3);
      goto p999;
      end;
    end;
   box(100,660,1600,400,146);

  outtextxy(100,660,currentdir2+filenames[sel+selstart,0]+' header:',230);

  outtextxy(100,680,'size: '+inttostr(head.size),246);

  outtextxy(100,700,'pcm type : '+inttostr(head.pcm),26);
  outtextxy(100,720,'channels: '+inttostr(head.channels),42);
  outtextxy(100,740,'sample rate: '+inttostr(head.srate),58);
  outtextxy(100,760,'bitrate: '+inttostr(head.brate),74);
  outtextxy(100,780,'bytes per sample: '+inttostr(head.bytesps),90);
  outtextxy(100,800,'bits per sample : '+inttostr(head.bps),106);
  outtextxy(100,820,'data size: '+inttostr(head.datasize),122);


 if (head.channels>2) or not((head.pcm=1) or (head.pcm=3)) then
  begin
  messagedlg('Error - wave format not known', MtError, [MbOk],0);
  fileclose(fh3);
  goto p999;
  end;
  ...
  if head.pcm=3 then src_format:=audio_f32
    ...
    else src_format:=audio_u8;
  convert:=true;
  end;

  wavepointer1:=getmem(head.datasize+33554432);
  currentwp:=wavepointer1;
  if wavepointer1=nil then
    begin
    messagedlg('Error - cannot allocate memory', MtError, [MbOk],0);
    fileclose(fh3);
    goto p999;
    end;
  outtextxy(100,840,'Loading wave file',138);
  ...
  if convert then
    ...
    if head.channels=1 then
      begin
      outtextxy(100,840,'Converting mono to stereo',138);
      currentdatasize*=2;
      wavepointer2:=getmem(currentdatasize+33554432);
 ...
 //     freemem(currentwp);
    ...
    if (head.pcm=1) then
      begin
      outtextxy(100,860,'Converting int to float',154);
      currentdatasize*=2;
      wavepointer3:=getmem(currentdatasize+33554432);
...
//      freemem(currentwp);
    ...
    head2.brate:=1536000;
    head2.channels:=2;
    fh3:=filecreate(currentdir2+filenames[sel+selstart,0]+'.32-192.wav');
...
if widespec.clicked=1 then begin
  ...
  widespectrum(wsfn,st,za,cl,over.selected); widespec.unselect; widespec.clicked:=0; end;
...
var fh:integer;
    buf:array[0..255] of integer;
    songname:string;
    i,j,k,oldx,x,pixel,pixel2:integer;
...
for i:=0 to time do filebuf[i]:=round(ffilebuf[i]) ;
 for i:=0 to 1750 do
    for j:=0 to 127 do
      message.putpixel(i,200-filebuf[128*i+j] div 256,178);
 message.redrawclient;
...
//for i:=0 to 1000 do form1.memo1.lines.add (inttostr((from+i)^[0])+' '+inttostr((from+i)^[1])+' '+floattostr((too+i)^[0])+' '+floattostr((too+i)^[1]));
end;



function resample44to192(from,too:Pfloatsample;il:integer):integer;
...
var i,j,k:integer;
    ...
    q,a1,a2:TFloatsample;

begin

//form1.progressbar1.position:=0;
for i:=0 to il+4194303 do (too+i)^:=zerosample;
i:=0;
k:=0;
l:=0;
repeat
  buf3:=buf2;
  buf1[0]:=(from+i)^*64;
  for j:=1 to 63 do buf1[j]:=zerosample;
  ...
  l-=64;
  i+=1;
if (i div 1000) mod 10 = 0 then begin box(276,880,40,16,146); outtextxy(276,880,inttostr(round(100*i/il))+'%',170);application.processmessages; end;
until i>il;
box(276,880,40,16,146); outtextxy(276,880,'100%',170);
application.processmessages;
result:=k;
end;
...
function cheby2(input:TFloatsample; mode:integer):Tfloatsample;
var i:integer;
...
// Low pass Chebyshev filter 20k @ 2822400 Hz
// computed with mkfilter http://www-users.cs.york.ac.uk/~fisher/mkfilter/trad.html
