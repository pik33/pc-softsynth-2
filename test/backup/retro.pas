...
for i:=(yres-cyres) div 2 to ((yres-cyres) div 2) +cyres-1 do
  begin
  for j:=0 to ((xres-cxres) div 2)-1 do begin if peek($70001)=0 then b:=lpeek($10000+4*((i div 4)+lpeek($60000) ) mod 1024); (pi+k)^:=b; k+=1; end;
  for j:=0 to cxres-1 do begin (pi+k)^:=raml^[$4000+ramw^[buf+l]]; k+=1; l+=1; end;
  for j:=0 to ((xres-cxres) div 2)-1 do begin (pi+k)^:=b; k+=1; end;
  end;
